...
from astm_classes.constants import ENCODING, ACK, EOT, NAK, ENQ
    ...
    if data == ENQ:
        ...
        while 1:
            ...
            if seq == counter:
                try:
                    if data[0] == 'P':
                        astm_data = Patient(*decode_record(data))
                        print(astm_data.practice_id)
                    elif data[0] == 'O':
                        astm_data = Order(*decode_record(data))
                        print(astm_data.sample_id)
                    elif data[0] == 'R':
                        astm_data = Result(*decode_record(data))
                        print("Result:"+astm_data.test.test_id +":" + astm_data.value)
                except:
                    s.send(NAK)  # Send Not Acknowledged message back to the sender on error
                    raise Rejected('Frame rejected %r' % seq + str(astm_data))
            else:
